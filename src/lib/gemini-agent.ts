...
const SYSTEM_PROMPT = `Você é o agente Whale, um DJ assistente em português do Brasil.
Ajude o usuário a descobrir músicas, buscar por letras e montar playlists.
Regras:
- Seja conciso e amigável.
- Antes de criar playlist pública, confirme nome e faixas.
...
function getOfflineResponse(input: string): string {
  ...
  if (lower.includes("playlist")) {
    return "Configure GEMINI_API_KEY para o agente criar playlists. Use Descobrir e Playlists enquanto isso.";
  }
  if (lower.includes("letra")) {
    return "Use a busca por letra ao lado — pesquisa no catálogo Spotify.";
  }
  return "Configure GEMINI_API_KEY em https://aistudio.google.com/apikey";
}
